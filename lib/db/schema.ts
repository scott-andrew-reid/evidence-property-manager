...
// Initialize Phase 2 v2 Schema
export async function initializeV2Schema() {
  const sql = getDb();
...
async function seedV2Data() {
  ...
  try {
    ...
    `;
  } catch (e) { console.log('Item types already seeded'); }
  ...
  try {
    await sql`
      ...
      ON CONFLICT (name) DO NOTHING
    `;
  } catch (e) { console.log('Locations already seeded'); }
  ...
  try {
    ...
    `;
  } catch (e) { console.log('Transfer reasons already seeded'); }
